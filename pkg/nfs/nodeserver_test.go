...
import (
	...
	"github.com/GoogleCloudPlatform/nfs-lb-csi-driver/test/utils/testutil"
...
func TestNodePublishVolume(t *testing.T) {
	...
	paramsWithoutServer := map[string]string{
		"share":               "share",
		mountPermissionsField: "0755",
	}
	...
	tests := []struct {
		...
		{
			desc: "[Success] Valid request with params without server IP",
			req: csi.NodePublishVolumeRequest{
				PublishContext: map[string]string{
					lbcontroller.NodeAnnotation: "10.10.10.10",
				},
				VolumeContext:    paramsWithoutServer,
				VolumeCapability: &csi.VolumeCapability{AccessMode: &volumeCap},
				VolumeId:         "vol_1",
				TargetPath:       targetTest},
			expectedErr: nil,
